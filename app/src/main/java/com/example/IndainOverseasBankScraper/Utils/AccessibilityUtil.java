package com.example.IndainOverseasBankScraper.Utils;
...
public class AccessibilityUtil {
    ...
    public static AccessibilityNodeInfo findNodeToResourceId(AccessibilityNodeInfo rootNode, String resourceId) {
        if (rootNode == null) return null;
        ...
        for (int i = 0; i < childCount; i++) {
            AccessibilityNodeInfo childNode = rootNode.getChild(i);
            if (childNode != null) {
                CharSequence viewId = childNode.getViewIdResourceName();
                if (viewId != null && viewId.equals(resourceId)) {
                    // Found the node with the specified resource ID
                    return childNode;
                }
                AccessibilityNodeInfo foundNode = findNodeToResourceId(childNode, resourceId); // Recursive call to search deeper
                if (foundNode != null) {
                    return foundNode;
                }
            }
    ...
    public static AccessibilityNodeInfo findOneText(AccessibilityNodeInfo rootNode, String text) {
        if (rootNode == null) return null;
        ...
        for (int i = 0; i < childCount; i++) {
            AccessibilityNodeInfo childNode = rootNode.getChild(i);
            AccessibilityNodeInfo foundNode = findOneText(childNode, text);
            if (foundNode != null) {
                return foundNode; // Return the node if found in the child subtree
            }
        }

        return null; // Return null if the text is not found in the subtree rooted at this node
    }
